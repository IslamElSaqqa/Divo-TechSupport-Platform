...
const Technician = () => {
    ...
    const handleStatusChange = async (requestId, newStatus) => {
        ...
        try {
            const response = await fetch(`/api/helpSession/technician/${requestId}/status`, {
                method: 'PATCH',
                headers: {
                    'Authorization': `Bearer ${technician.token}`,
                    'Content-Type': 'application/json'
    ...
    const handleTypeChange = async (requestId, newType) => {
        ...
        try {
            const response = await fetch(`/api/helpSession/technician/${requestId}/type`, {
                method: 'PATCH',
                headers: {
                    'Authorization': `Bearer ${technician.token}`,
                    'Content-Type': 'application/json'
    ...
    const handleAddNote = () => {
        ...
        if (!selectedRequest || selectedRequest.specialist?._id !== technician._id) {
    ...
    const handleNoteSubmit = async () => {
        ...
        try {
            const response = await fetch(`/api/helpSession/technician/${selectedRequest._id}/note`, {
                method: 'PATCH',
                headers: {
                    'Authorization': `Bearer ${technician.token}`,
                    'Content-Type': 'application/json'
                    ...
                    <div className={styles.bottomSection}>
                        ...
                        {!selectedRequest ? (
                            <>
                                <div className={styles.filters}>
                                    <button
                                        ...
                                        className={`${styles.filtersButton} ${activeFilter === "Pending" ? styles.activeFilter : ""}`}
                                    >
                                        All Requests
                                    </button>
                                        ...
                                        <tbody>
                                            {filteredData.map((row) => (
                                                <tr key={row._id} className={styles.tableRow}>
                                                    ...
                                                    <td className={styles.tableCell}>
                                                        <span className={`${styles.statusBadge} ${row.status.toLowerCase() === 'pending' ? styles.statusBadgePending :
                                                    ...
                                                    <td className={styles.tableCell}>
                                                        {row.status === "pending" ? (
                                                            ...
                                                            <div className={styles.rejectButton}>Rejected</div>
                                                        ) : (
                                                            <button className={styles.resetButton} onClick={() => handleStatusChange(row._id, "rejected")}>Cancel</button>
                                                        )}
                                                    </td>
                                                </tr>
                                            ))}
                            ...
                            <div className={styles.requestDetails}>
                                    ...
                                    {selectedType ? (
                                        ...
                                        <div className={styles.typeOptions}>
                                            <button
                                                ...
                                                onClick={() => handleTypeChange(selectedRequest._id, 'long')}
                                            >
                                                Long
                                            </button>
                                            <button
                                                className={styles.typeButton}
                                                onClick={() => handleTypeChange(selectedRequest._id, 'short')}
                                            >
                                                Short
                                            </button>
                                        </div>
                                ...
                                <div className={styles.detailItem}><strong>Phone Number:</strong> {selectedRequest.user ? selectedRequest.user.phone_number : "No phone number"}</div>
                                <div className={styles.detailItem}><strong>Note:</strong> {selectedRequest.note || ""}</div>
                                <div className={styles.detailItem}><strong>Status:</strong> {selectedRequest.status}</div>
                                <div className={styles.detailItem}><strong>Image:</strong> <img src={selectedRequest.image_url || ""}
                                    alt="Zoomable"
                                    style={{ cursor: "pointer" }}
                                    onClick={() => setIsZoomed(true)}></img>
                                    {isZoomed && (
                                        <div
                                            style={{
                                                position: "fixed",
                                ...
                                <div className={styles.detailItem}><strong>Assigned Technician:</strong> {selectedRequest.specialist?.specialist_name || "Not assigned"}</div>
