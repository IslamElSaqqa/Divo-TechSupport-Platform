...
const StoreCategories = () => {
      ...
      try {
        ...
        if (!scrapedData || Object.keys(scrapedData).length === 0) {
          const response = await axios.get(
            `http://localhost:4000/api/scraping/scrapeAll?productName=${productName}`
          );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:4000/api/scraping/scrapeAll?productName=${inputProduct}`
      );
      setScrapedData(response.data);
    } catch (err) {
  ...
  const scrollContainers = {
    amazon: React.useRef(null),
    jumia: React.useRef(null),
    cairoSales: React.useRef(null),
    shikoStore: React.useRef(null),
    maxHardware: React.useRef(null),
  };

  const scrollLeft = (store) => {
    if (scrollContainers[store]?.current) {
      scrollContainers[store].current.scrollBy({
        left: -300,
        behavior: "smooth",
      });
    }
  ...
  return (
    <div className="content-container">
      <div className="search-container-error">
        ...
        <span className="Store-welcome-span">
          "Find The Best Prices For Your Product"
        </span>

        {["amazon", "jumia", "cairoSales", "shikoStore", "maxHardware"].map(
          (store) => (
                  ...
                  <div className="sales-header">
                    ...
                    <div className="navigation-controls">
                      <button
                        ...
                        onClick={() => scrollLeft(store)}
                      >
                        <ChevronLeft size={24} />
                      </button>
                      <button
                        className="nav-button"
                        onClick={() => scrollRight(store)}
                      >
